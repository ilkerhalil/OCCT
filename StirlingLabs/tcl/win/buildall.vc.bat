...
nmake -nologo -f makefile.vc release install OPTS=%OPTS% %1
if errorlevel 1 goto error

:: Build the static core and shell.
::
REM set OPTS=static,msvcrt
REM if not %SYMBOLS%.==. set OPTS=symbols,static,msvcrt
REM nmake -nologo -f makefile.vc shell OPTS=%OPTS% %1
